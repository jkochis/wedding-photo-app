...
export class PhotoManager {
    ...
    setupStateSubscriptions() {
        ...
        this.subscriptions.push(state.subscribe('currentFilter', () => this.updateFilteredPhotos()), state.subscribe('selectedPerson', () => this.updateFilteredPhotos()));
    }
    /**
    ...
    updateFilteredPhotos() {
        ...
        // Apply people filter
        if (selectedPerson) {
            filtered = filtered.filter(photo => photo.people &&
     ...
     */
    getPhotosByPerson(person) {
        return this.photos.filter(photo => photo.people &&
            Array.isArray(photo.people) &&
    ...
    searchPhotos(query) {
        ...
        return this.photos.filter(photo => {
            ...
            // Search in people
            if (photo.people && Array.isArray(photo.people)) {
                return photo.people.some(person => person.toLowerCase().includes(searchTerm));
            }
            // Search in filename
